...
def set_libraries_for_source_file(source_file, libraries, source_map):
    ...
    # if there's only one library specified and it's the New Relic library,
    # don't hash the source file
...
def gen_docs_site_doc(license_info):
    ...
    for library in sorted(license_info["libraries"]):
        ...
        for license in sorted(license_info["libraries"][library]["licenses"]):
            ...
            # if the url ends in .html.erb, we strip it so it works with
            # the docs site. we also will append a "/"
...
def gen_installer_doc_command(args):
    ...
    if not noreview:
        # first, let's do a review to make sure all files, libraries,
...
def get_flattened_file_list(root_dir, ignored_files, repo_files):
    ...
    for root, dirs, files in os.walk(root_dir):
        for name in files:
            # get the path to the file from the root_dir since we normalize
        ...
        # note: the directories can't be removed while looping over them
        # or the list gets funky/out of sync
        dirs_to_remove = []
...
def check_local_license_urls(license_info):
    ...
    for license in license_info["licenses"]:
        # the url can either be a full remote url or a relative path on
        # the new relic site. decode it and see what type it is
