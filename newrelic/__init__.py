...
# for the same named application.
...
# that timing is started and stopped appropriately. The latter
# is quite tricky as need to attach it to when the 'close()'
# method of any generator returned by an WSGI application is
# called. Only way to do this is to wrap response from the WSGI
# application with a generator with it having a 'close()' method
# which in turn executes the wrapped generators 'close()' method
# when it exists and then executing and finalisation.
