...
def test_fetch_file_valid(case):
    ...
    with mock.patch('newrelic.common.utilization.open', fake_open,
            create=True):
        result = u.DockerUtilization.fetch()
...
@mock.patch.object(u, 'open', create=True)
...
def test_get_values_endswith_slash():
    result = u.DockerUtilization.get_values('/')
...
def test_valid_chars_none(validate_error_metric_forgone):
    result = u.DockerUtilization.valid_chars(None)
    assert result is False


def test_valid_chars_true(validate_error_metric_forgone):
    result = u.DockerUtilization.valid_chars('abc123')
    assert result is True


def test_valid_chars_invalid(validate_error_metric_forgone):
    result = u.DockerUtilization.valid_chars('cats')
    assert result is False
...
def test_valid_length_none(validate_error_metric_forgone):
    result = u.DockerUtilization.valid_length(None)
    assert result is False


def test_valid_length_true(validate_error_metric_forgone):
    data = '0' * 64
    result = u.DockerUtilization.valid_length(data)
    assert result is True
