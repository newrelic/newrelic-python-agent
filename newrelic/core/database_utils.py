...
_parse_identifier_p = "".join(
    (
        ...
        "|",
        _parse_identifier_4_p,
        "|",
        _parse_identifier_5_p,
        "|",
        _parse_identifier_6_p,
        "|",
        _parse_identifier_7_p,
        ")",
    )
)
...
_explain_plan_postgresql_re_1_mask_true = re.compile(
    r"""((?P<double_quotes>"[^"]*")|""" r"""(?P<single_quotes>'([^']|'')*'))"""
)
...
def _explain_plan(connections, sql, database, connect_params, cursor_params, sql_parameters, execute_params):
    ...
    try:
        ...
        if settings.debug.log_explain_plan_queries:
            _logger.exception(
                "Error occurred when executing explain "
