...
class LoopNode(_LoopNode, GenericNodeMixin):
    ...
    def time_metrics(self, stats, root, parent):
        ...
        yield TimeMetric(name=name, scope="", duration=self.duration, exclusive=self.duration)
        ...
        yield TimeMetric(name=name, scope="", duration=self.duration, exclusive=None)

        if root.type == "WebTransaction":
            yield TimeMetric(name=f"{name}Web", scope="", duration=self.duration, exclusive=None)
        else:
            yield TimeMetric(name=f"{name}Other", scope="", duration=self.duration, exclusive=None)
    ...
    def trace_node(self, stats, root, connections):
        ...
        # Agent attributes
        params = {"exclusive_duration_millis": 1000.0 * self.duration}
