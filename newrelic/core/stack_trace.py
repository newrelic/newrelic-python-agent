...
def _format_stack_trace(frames):
    ...
    result.extend(['File "{source}", line {line}, in {name}'.format(**d) for d in frames])
    return result
...
def _extract_stack(f, skip, limit):
    ...
    while f is not None and n < limit:
        list.append(dict(source=f.f_code.co_filename, line=f.f_lineno, name=f.f_code.co_name))
...
def _extract_tb(tb, limit):
    ...
    while tb is not None and n < limit:
        f = tb.tb_frame
        list.append(dict(source=f.f_code.co_filename, line=tb.tb_lineno, name=f.f_code.co_name))
