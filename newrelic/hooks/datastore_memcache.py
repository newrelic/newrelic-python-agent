...
def wrap_memcache_single(module, object_path, product, target, operation):
    wrap_object(module.Client, object_path, MemcacheSingleWrapper, (product, target, operation, module))
...
_memcache_multi_methods = ("delete_multi", "get_multi", "set_multi", "get_stats", "get_slabs", "flush_all")
...
def instrument_memcache(module):
    ...
    for name in _memcache_client_methods:
        if hasattr(module.Client, name):
            wrap_memcache_single(module, name, product="Memcached", target=None, operation=name)

    for name in _memcache_multi_methods:
        if hasattr(module.Client, name):
            wrap_datastore_trace(module.Client, name, product="Memcached", target=None, operation=name)
