...
def _wrap_Valkey_method_wrapper_(module, instance_class_name, operation):
    name = f"{instance_class_name}.{operation}"
    if operation in _valkey_client_gen_methods:
...
def _wrap_asyncio_Valkey_method_wrapper(module, instance_class_name, operation):
    def _nr_wrapper_asyncio_Valkey_method_(wrapped, instance, args, kwargs):
    ...
    wrap_function_wrapper(module, name, _nr_wrapper_asyncio_Valkey_method_)
...
async def wrap_async_Connection_send_command(wrapped, instance, args, kwargs):
    ...
    # Older Valkey clients would when sending multi part commands pass
    ...
    if (
        operation.split()[0] not in _valkey_multipart_commands
    ...
    if operation in _valkey_multipart_commands and len(args) > 1:
...
def _nr_Connection_send_command_wrapper_(wrapped, instance, args, kwargs):
    ...
    if operation.split()[0] not in _valkey_multipart_commands:
        return wrapped(*args, **kwargs)
...
def instrument_valkey_client(module):
    if hasattr(module, "StrictValkey"):
        for name in _valkey_client_methods:
            if name in vars(module.StrictValkey):
                _wrap_Valkey_method_wrapper_(module, "StrictValkey", name)

    if hasattr(module, "Valkey"):
        for name in _valkey_client_methods:
            if name in vars(module.Valkey):
                _wrap_Valkey_method_wrapper_(module, "Valkey", name)
...
def instrument_valkey_commands_core(module):
    _instrument_valkey_commands_module(module, "CoreCommands")


def instrument_valkey_commands_sentinel(module):
    _instrument_valkey_commands_module(module, "SentinelCommands")


def instrument_valkey_commands_json_commands(module):
    _instrument_valkey_commands_module(module, "JSONCommands")


def instrument_valkey_commands_search_commands(module):
    _instrument_valkey_commands_module(module, "SearchCommands")


def instrument_valkey_commands_timeseries_commands(module):
    _instrument_valkey_commands_module(module, "TimeSeriesCommands")


def instrument_valkey_commands_graph_commands(module):
    _instrument_valkey_commands_module(module, "GraphCommands")


def instrument_valkey_commands_bf_commands(module):
    _instrument_valkey_commands_module(module, "BFCommands")
    _instrument_valkey_commands_module(module, "CFCommands")
    _instrument_valkey_commands_module(module, "CMSCommands")
    _instrument_valkey_commands_module(module, "TDigestCommands")
    _instrument_valkey_commands_module(module, "TOPKCommands")


def instrument_valkey_commands_cluster(module):
    _instrument_valkey_commands_module(module, "ValkeyClusterCommands")


def _instrument_valkey_commands_module(module, class_name):
    for name in _valkey_client_methods:
        if hasattr(module, class_name):
            class_instance = getattr(module, class_name)
            if hasattr(class_instance, name):
                _wrap_Valkey_method_wrapper_(module, class_name, name)
