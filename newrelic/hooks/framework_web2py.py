...
def instrument_gluon_compileapp(module):
    ...
    def transaction_name_run_models_in(environment):
        return "%s::%s" % (environment["request"].application, environment["response"].view)

    newrelic.api.transaction_name.wrap_transaction_name(
        module, "run_models_in", name=transaction_name_run_models_in, group="Web2Py"
    )
    ...
    newrelic.api.function_trace.wrap_function_trace(
        module, "run_models_in", name=name_function_run_models_in, group="Python/Web2Py/Models"
    )
    ...
    newrelic.api.function_trace.wrap_function_trace(
        module, "run_controller_in", name=name_function_run_controller_in, group="Python/Web2Py/Controller"
    )
    ...
    newrelic.api.function_trace.wrap_function_trace(
        module, "run_view_in", name=name_function_run_view_in, group="Python/Web2Py/View"
    )
...
def instrument_gluon_restricted(module):
    ...
    newrelic.api.function_trace.wrap_function_trace(
        module, "restricted", name=name_function_restricted, group=group_function_restricted
    )
...
def instrument_gluon_main(module):
    ...
    newrelic.api.object_wrapper.wrap_object(module, "serve_controller", error_serve_controller)
...
def instrument_gluon_template(module):
    ...
    def name_function_parse_template(filename, path="views/", context=dict(), *args, **kwargs):
    ...
    newrelic.api.function_trace.wrap_function_trace(
        module, "parse_template", name=name_function_parse_template, group="Template/Compile"
    )
...
def instrument_gluon_tools(module):
    ...
    newrelic.api.external_trace.wrap_external_trace(
        module, "fetch", library="gluon.tools.fetch", url=url_external_fetch
    )
    ...
    newrelic.api.external_trace.wrap_external_trace(
        module, "geocode", library="gluon.tools.geocode", url="http://maps.google.com/maps/geo"
    )
...
def instrument_gluon_http(module):
    ...
    newrelic.api.pre_function.wrap_pre_function(module, "HTTP.to", transaction_name_name_not_found)
