...
from newrelic.common.encoding_utils import PARENT_TYPE, NrTraceState, W3CTraceState
...
def wrap_start_internal_or_server_span(wrapped, instance, args, kwargs):
    ...
    if context_carrier:
        if ("HTTP_HOST" in context_carrier) or ("http_version" in context_carrier):
            ...
            if ("tracestate" in nr_headers) and ("@nr=" not in nr_headers["tracestate"]):
                # The tracestate has been generated by OTel and is in OTel format
                otel_tracestate = nr_headers.pop("tracestate", None)
                tracestate_dict = W3CTraceState.decode(otel_tracestate)
                
                # Convert sampled, priority, and timestamp data types
                tracestate_dict["sa"] = bool(tracestate_dict.get("sa"))
