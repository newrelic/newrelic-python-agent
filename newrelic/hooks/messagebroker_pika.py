...
def _add_consume_rabbitmq_trace(transaction, method, properties, nr_start_time, queue_name=None, channel=None):
    ...
    transaction.start_time = min(nr_start_time, transaction.start_time)
...
def _bind_basic_publish(exchange, routing_key, body, properties=None, *args, **kwargs):
...
def _nr_wrapper_basic_publish(wrapped, instance, args, kwargs):
    ...
    (exchange, routing_key, body, properties, args, kwargs) = _bind_basic_publish(*args, **kwargs)
...
def instrument_pika_adapters(module):
    ...
    if hasattr(module, "tornado_connection"):
        wrap_function_wrapper(module, "tornado_connection.TornadoConnection.channel", _disable_channel_transactions)


def instrument_pika_spec(module):
    wrap_function_wrapper(module, "Basic.Deliver.__init__", _nr_wrapper_Basic_Deliver_init_)
...
def instrument_pika_channel(module):
    ...
    _wrap_Channel_consume_callback(module, "Channel.basic_consume", wrap_consume)
