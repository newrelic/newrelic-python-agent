...
GEMINI_VERSION = get_package_version("google.generativeai")
GEMINI_VERSION_TUPLE = tuple(map(int, GEMINI_VERSION.split(".")))
GEMINI_V1 = GEMINI_VERSION_TUPLE >= (1,)
...
def _record_embedding_success(transaction, embedding_id, linking_metadata, kwargs, ft, response):
    ...
    try:
        ...
        organization = (
            response_headers.get("gemini-organization")
            if GEMINI_V1
        ...
        full_embedding_response_dict = {
            ...
            "response.headers.llmVersion": response_headers.get("gemini-version"),
...
def _record_embedding_error(transaction, embedding_id, linking_metadata, kwargs, ft, exc):
    ...
    try:
        if GEMINI_V1:
            ...
            exc_organization = response_headers.get("gemini-organization")
...
def _handle_completion_success(transaction, linking_metadata, completion_id, kwargs, ft, return_val):
    ...
    if stream:
        try:
            ...
            setattr(return_val, "_nr_gemini_attrs", getattr(return_val, "_nr_gemini_attrs", {}))
            return_val._nr_gemini_attrs["messages"] = kwargs.get("messages", [])
            return_val._nr_gemini_attrs["temperature"] = kwargs.get("temperature")
            return_val._nr_gemini_attrs["max_tokens"] = kwargs.get("max_tokens")
            return_val._nr_gemini_attrs["model"] = kwargs.get("model") or kwargs.get("engine")
            return
        except Exception:
    ...
    try:
        ...
        if response:
            if "model" in response:
                response_model = responseDict.get("model")
            if "id" in response:
                response_id = responseDict.get("id")
            if "usage" in response:
                response_usage = responseDict.get("usage") or {}
            output_message_list = []
            finish_reason = None
            candidates = responseDict.get("candidates") or []
            if candidates:
                output_message_list = [
                    candidates[0].get("message") or {"content": candidates[0].get("content").get("parts")[0].get("text"), "role": "assistant"}
                ]
                finish_reason = candidates[0].get("finish_reason")

            prompt_token_count = responseDict.get("usage_metadata").get("prompt_token_count")
            candidates_token_count = responseDict.get("usage_metadata").get("candidates_token_count")
        else:
            ...
            if "content" in kwargs:
                output_message_list = [{"content": kwargs.candidates.get("content"), "role": kwargs.get("role")}]
        ...
        request_id = response_headers.get("x-request-id")
        organization = response_headers.get("gemini-organization") or getattr(response, "organization", None)
        ...
        full_chat_completion_summary_dict = {
            ...
            "response.candidates.finish_reason": finish_reason,
            "response.headers.llmVersion": response_headers.get("gemini-version"),
            "response.headers.ratelimitLimitRequests": check_rate_limit_header(
                response_headers, "x-ratelimit-limit-requests", True
            ),
...
def wrap_convert_to_gemini_object(wrapped, instance, args, kwargs):
    ...
    if isinstance(returned_response, genai.gemini_object.GeminiObject) and isinstance(
        resp, genai.gemini_response.GeminiResponse
    ):
    ...
    if return_val:
        try:
            if GEMINI_V1:
                ...
                self._nr_gemini_attrs["response_headers"] = getattr(self, "_nr_response_headers", {})
            else:
                self._nr_gemini_attrs["response_headers"] = getattr(return_val, "_nr_response_headers", {})
            candidates = return_val.get("candidates") or []
            self._nr_gemini_attrs["response.model"] = return_val.get("model")
            self._nr_gemini_attrs["id"] = return_val.get("id")
            self._nr_gemini_attrs["response.organization"] = return_val.get("organization")
            if candidates:
                delta = candidates[0].get("delta") or {}
                if delta:
                    self._nr_gemini_attrs["content"] = self._nr_gemini_attrs.get("content", "") + (
                        delta.get("content") or ""
                    )
                    self._nr_gemini_attrs["role"] = self._nr_gemini_attrs.get("role") or delta.get("role")
                self._nr_gemini_attrs["finish_reason"] = candidates[0].get("finish_reason")
        except Exception:
            _logger.warning(STREAM_PARSING_FAILURE_LOG_MESSAGE % traceback.format_exception(*sys.exc_info()))
...
def _record_events_on_stop_iteration(self, transaction):
    if hasattr(self, "_nr_ft"):
        ...
        try:
            gemini_attrs = getattr(self, "_nr_gemini_attrs", {})
            ...
            completion_id = str(uuid.uuid4())
            response_headers = gemini_attrs.get("response_headers") or {}
            _record_completion_success(
                transaction, linking_metadata, completion_id, gemini_attrs, self._nr_ft, response_headers, None
            )
        except Exception:
            _logger.warning(RECORD_EVENTS_FAILURE_LOG_MESSAGE % traceback.format_exception(*sys.exc_info()))
        finally:
            # Clear cached data as this can be very large.
            # Note this is also important for not reporting the events twice. In gemini v1
            ...
            if hasattr(self, "_nr_gemini_attrs"):
                self._nr_gemini_attrs.clear()
...
def _handle_streaming_completion_error(self, transaction, exc):
    if hasattr(self, "_nr_ft"):
        ...
        # If there are no gemini attrs exit early as there's no data to record.
        if not gemini_attrs:
            self._nr_ft.__exit__(*sys.exc_info())
            return
        linking_metadata = get_trace_linking_metadata()
        completion_id = str(uuid.uuid4())
        _record_completion_error(transaction, linking_metadata, completion_id, gemini_attrs, self._nr_ft, exc)
...
def set_attrs_on_generator_proxy(proxy, instance):
    ...
    if hasattr(instance, "_nr_gemini_attrs"):
        proxy._nr_gemini_attrs = instance._nr_gemini_attrs
...
def instrument_gemini_api_resources_embedding(module):
...
def instrument_gemini_api_resources_chat_completion(module):
...
def instrument_gemini_resources_chat_completions(module):
...
def instrument_gemini_resources_embeddings(module):
...
def instrument_gemini_util(module):
    if hasattr(module, "convert_to_gemini_object"):
        wrap_function_wrapper(module, "convert_to_gemini_object", wrap_convert_to_gemini_object)
        # This is to mark where we instrument so the SDK knows not to instrument them
        # again.
        setattr(module.convert_to_gemini_object, "_nr_wrapped", True)


def instrument_gemini_base_client(module):
...
def instrument_gemini_api_resources_abstract_engine_api_resource(module):
...
def instrument_ç__streaming(module):
