...
def _import_hook(module, function):
    def _instrument(target):
        _newrelic.log(_newrelic.LOG_INFO, "execute import-hook %s" % ((target,
                module, function),))
...
def _process_import_hook(target, module, function='instrument'):
    ...
    if newrelic.config.config_object.has_section(section):
        try:
            ...
            pass
    if enabled and not newrelic.config.config_object.has_option(section, 'execute'):
...
for section in newrelic.config.config_object.sections():
    if section.startswith('import-hook:'):
        ...
        try:
            ...
            if enabled:
                try:
                    parts = newrelic.config.config_object.get(section, 'execute').split(':')
                except ConfigParser.NoOptionError:
                    pass
                else:
                    module = parts[0]
