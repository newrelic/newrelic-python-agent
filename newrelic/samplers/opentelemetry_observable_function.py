...
# from newrelic.core.stats_engine import CustomMetrics, DimensionalMetrics
# # from newrelic.core.agent import register_data_source
...
class _ObservableFunction:
    def __init__(self, callback, function_type="counter", name=None, attributes=None):
        self.function_type = function_type  # options: "counter", "updown", "gauge"
    ...
    def __call__(self):
        # breakpoint()
        _logger.debug(f"TRAVERSE=newrelic.samplers.opentelemetry_observable_function--self.callback: {self.callback}")
        for observation in self.callback(CallbackOptions):
            ...
            if self.function_type == "counter":
                if observation.value < 0:
                    raise ValueError("Amount used in counter %s must be a non-negative number.", observation.value)
                yield (f"{self.name}/{sub_name}", {"count": observation.value}, self.attributes)
            elif self.function_type == "updown":
                yield (f"{self.name}/{sub_name}", {"count": observation.value}, self.attributes)
            elif self.function_type == "gauge":
                yield (f"{self.name}/{sub_name}", observation.value, self.attributes)
            else:
                yield ()
        
        
observable_function = _ObservableFunction



# observable_counter = _ObservableCounter
# # register_data_source(observable_counter)    
...
# #                 sub_name = "/".join([f"{k}/{v}" for k, v in observation.attributes.items()])
# #                 yield (f"{self.name}/{sub_name}", {"count": observation.value})
