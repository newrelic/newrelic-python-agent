...
target_application = webtest.TestApp('http://localhost:8002')

def test_pywsgi_application_index():
    for i in range(3):
        response = target_application.get('/pywsgi')
        response.mustcontain('WSGI RESPONSE')

def test_pywsgi_request_timeout_application():
    for i in range(3):
        response = target_application.get(
                '/request-timeout-application/pywsgi', status=500)

def test_pywsgi_request_timeout_response():
...
def test_pywsgi_request_timeout_finalize():
...
def test_pywsgi_raise_exception_application():
    for i in range(3):
        response = target_application.get(
                '/raise-exception-application/pywsgi', status=500)

def test_pywsgi_raise_exception_response():
    for i in range(3):
        response = target_application.get(
                '/raise-exception-response/pywsgi', status=500)

def test_pywsgi_raise_exception_finalize():
    for i in range(3):
        response = target_application.get(
