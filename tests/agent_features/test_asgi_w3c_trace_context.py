...
LONG_TRACESTATE = ",".join(["{}@rojo=f06a0ba902b7".format(x) for x in range(32)])
INBOUND_UNTRUSTED_NR_TRACESTATE = (
    "2@nr=0-0-1345936-55632452-27jjj2d8890283b4-b28ce285632jjhl9-" "1-1.1273-1569367663277"
)
...
INBOUND_NR_TRACESTATE_INVALID_TIMESTAMP = (
    "1@nr=0-0-1349956-41346604-27ddd2d8890283b4-b28be285632bbc0a-" "0-0.4-timestamp"
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (LONG_TRACESTATE + "," + INBOUND_NR_TRACESTATE, ",".join("{}@rojo=f06a0ba902b7".format(x) for x in range(31))),
    ),
    ids=(
...
@pytest.mark.parametrize(
    "inbound_traceparent,span_events_enabled",
    (
        (True, True),
        (True, False),
        (False, True),
...
def test_inbound_traceparent_header(traceparent, intrinsics, metrics):
    ...
    def _test():
        test_asgi_application.make_request("GET", "/", headers={"traceparent": traceparent})
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("rojo=12345," + "v" * 257 + "=x", {"tracingVendors": "rojo"}),
        ("rojo=12345,k=" + "v" * 257, {"tracingVendors": "rojo"}),
    ),
)
@override_application_settings(_override_settings)
def test_inbound_tracestate_header(tracestate, intrinsics):
    ...
    def _test():
        test_asgi_application.make_request(
            "GET",
            "/",
            headers={
                "traceparent": INBOUND_TRACEPARENT,
