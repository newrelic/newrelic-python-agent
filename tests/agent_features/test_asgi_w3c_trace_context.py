...
def test_tracestate_generation(inbound_nr_tracestate):
    ...
    for header_name, header_value in response.headers.items():  # noqa: B007
...
def test_inbound_tracestate_header(tracestate, intrinsics):
    ...
    def _test():
        test_asgi_application.make_request(
            "GET", "/", headers={"traceparent": INBOUND_TRACEPARENT, "tracestate": tracestate}
        )
...
def test_w3c_tracestate_header():
    ...
    def _test():
        test_asgi_application.make_request(
            "GET", "/", headers={"traceparent": INBOUND_TRACEPARENT, "tracestate": INBOUND_TRACESTATE}
        )
