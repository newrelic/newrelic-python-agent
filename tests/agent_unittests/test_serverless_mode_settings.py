...
SERVERLESS_MODE_ENV = {"NEW_RELIC_SERVERLESS_MODE_ENABLED": "true"}
...
APP_NAME_ENV = {"NEW_RELIC_APP_NAME": "c;d"}


@pytest.mark.parametrize(
    "ini,env,serverless_mode",
    [
...
def test_serverless_mode_environment(ini, env, serverless_mode, global_settings):
...
@pytest.mark.parametrize(
    "ini,env,values",
    [
        (INI_FILE_EMPTY, NON_SERVERLESS_MODE_ENV, (None, "Unknown", None)),
        (INI_FILE_SERVERLESS_MODE, DT_ENV, ("account_id", "application_id", "trusted_key")),
    ],
)
...
@pytest.mark.parametrize(
    "ini,env,register_name,handler_name",
    (
...
def test_serverless_mutli_app_names(ini, env, register_name, handler_name, global_settings):
