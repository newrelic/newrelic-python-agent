...
import pytest
from testing_support.fixtures import failing_endpoint, override_application_settings, override_generic_settings
...
def load_tests():
    ...
    for test in tests:
        ...
        parent_not_sampled_matching_acct_id_sampled_true = test.pop(
    ...
    def _test(test_adaptive=False, test_totals=False, num_tests=1):
        ...
        if test_totals:
            if expected_sampled is not None:
                assert (
                    expected_sampled - expected_sampled * variance
                    <= total / num_tests
                    <= expected_sampled + expected_sampled * variance
                )
            if expected_sampled_partial is not None:
                assert (
                    expected_sampled_partial - expected_sampled_partial * variance
                    <= partial / num_tests
                    <= expected_sampled_partial + expected_sampled_partial * variance
                )
            if expected_sampled_full is not None:
                assert (
                    expected_sampled_full - expected_sampled_full * variance
                    <= (total - partial) / num_tests
                    <= expected_sampled_full + expected_sampled_full * variance
                )

        if test_adaptive and expected_adaptive_sampler_decisions is not None:
            assert (
                expected_adaptive_sampler_decisions - expected_adaptive_sampler_decisions * variance
                <= adaptive_sampling_decisons
                <= expected_adaptive_sampler_decisions + expected_adaptive_sampler_decisions * variance
            )
