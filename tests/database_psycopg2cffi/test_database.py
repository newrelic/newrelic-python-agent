...
def validate_transaction_metrics(scope):
    ...
    def _validate_transaction_metrics(wrapped, instance, args, kwargs):
        try:
            ...
            assert metrics[
                    ('Database/database_psycopg2cffi/select', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/select', scope)].call_count == 1

            assert metrics[('Database/insert', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/insert', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/insert', scope)].call_count == 1

            assert metrics[('Database/update', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/update', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/update', scope)].call_count == 1

            assert metrics[('Database/delete', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/delete', '')].call_count == 1
            assert metrics[
                    ('Database/database_psycopg2cffi/delete', scope)].call_count == 1
