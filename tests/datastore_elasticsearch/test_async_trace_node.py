...
async def _exercise_es_v7(es):
    await es.index(
        index="contacts", doc_type="person", body={"name": "Joe Tester", "age": 25, "title": "QA Master"}, id=1
    )


async def _exercise_es_v8(es):
    await es.index(index="contacts", body={"name": "Joe Tester", "age": 25, "title": "QA Master"}, id=1)
...
@background_task()
def test_async_trace_node_datastore_params_enable_instance(async_client, loop):
...
@validate_tt_parenting(_tt_parenting)
@background_task()
def test_async_trace_node_datastore_params_disable_instance(async_client, loop):
    loop.run_until_complete(_exercise_es(async_client))
...
@validate_tt_parenting(_tt_parenting)
@background_task()
def test_async_trace_node_datastore_params_instance_only(async_client, loop):
    loop.run_until_complete(_exercise_es(async_client))
