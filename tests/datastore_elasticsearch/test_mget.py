...
def client():
    ...
    # dbs.
    if ES_VERSION >= (8,):
        client = Elasticsearch(urls, node_selector_class=RoundRobinSelector,
...
def _exercise_es_multi(es):
    ...
    if ES_VERSION >= (8,):
        ...
        # set on db 2
        es.index(index='contacts', body={'name': 'Jane Tester', 'age': 22, 'title': 'Senior QA Engineer'},
                id=2)
    else:
        es.index(index='contacts', doc_type='person', body={'name': 'Joe Tester', 'age': 25, 'title': 'QA Engineer'}, id=1)
        # set on db 2
        es.index(index='contacts', doc_type='person', body={'name': 'Jane Tester', 'age': 22, 'title': 'Senior QA Engineer'}, id=2)
