...
from opensearchpy.connection_pool import RoundRobinSelector
...
_base_rollup_metrics = (
    ...
    ("Datastore/OpenSearch/all", 3),
    ("Datastore/OpenSearch/allOther", 3),
    ("Datastore/operation/OpenSearch/index", 2),
    ("Datastore/operation/OpenSearch/mget", 1),
...
def client(loop):
    urls = [f"http://{db['host']}:{db['port']}" for db in OPENSEARCH_MULTIPLE_SETTINGS]
    ...
    # doing two db calls will mean opensearch is talking to two different dbs.
    _client = AsyncOpenSearch(urls, selector_class=RoundRobinSelector, randomize_hosts=False)
...
def test_async_multi_get_enabled(client, loop):
    loop.run_until_complete(_exercise_opensearch_multi(client))
