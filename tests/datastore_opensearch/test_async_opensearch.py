...
async def _exercise_opensearch(client):
    ...
    await client.index(index="contacts", body={"name": "Jessica Coder", "age": 32, "title": "Programmer"}, id=2)
    await client.index(index="contacts", body={"name": "Freddy Tester", "age": 29, "title": "Assistant"}, id=3)
    await client.indices.refresh(index="contacts")
    await client.index(index="address", body={"name": "Sherlock", "address": "221B Baker Street, London"}, id=1)
    await client.index(
        index="address", body={"name": "Bilbo", "address": "Bag End, Bagshot row, Hobbiton, Shire"}, id=2
    )
    await client.search(index="contacts", q="name:Joe")
    await client.search(index="contacts", q="name:jessica")
    await client.search(index="address", q="name:Sherlock")
    await client.search(index=["contacts", "address"], q="name:Bilbo")
    await client.search(index="contacts,address", q="name:Bilbo")
    await client.search(index="*", q="name:Bilbo")
    await client.search(q="name:Bilbo")
...
@validate_transaction_metrics(
    "test_async_opensearch:test_async_opensearch_operation_disabled",
...
@background_task()
def test_async_opensearch_operation_disabled(async_client, loop):
    loop.run_until_complete(_exercise_opensearch(async_client))


@validate_transaction_errors(errors=[])
@validate_transaction_metrics(
    "test_async_opensearch:test_async_opensearch_operation_enabled",
...
@background_task()
def test_async_opensearch_operation_enabled(async_client, loop):
    loop.run_until_complete(_exercise_opensearch(async_client))


@validate_transaction_errors(errors=[])
@validate_transaction_metrics(
    "test_async_opensearch:test_async_opensearch_operation_enabled_empty_transaction_settings",
    scoped_metrics=_enable_scoped_metrics,
    rollup_metrics=_enable_rollup_metrics,
    background_task=True,
)
@override_application_settings(_enable_instance_settings)
@background_task()
def test_async_opensearch_operation_enabled_empty_transaction_settings(async_client, loop):
...
def test_async_opensearch_no_transaction(async_client, loop):
    loop.run_until_complete(_exercise_opensearch(async_client))
