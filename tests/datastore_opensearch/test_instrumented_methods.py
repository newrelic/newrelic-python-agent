...
import opensearchpy
import opensearchpy.client as client_module
import pytest
...
def _all_client_classes():
    classes = [opensearchpy.OpenSearch]
    for name in dir(client_module):
        obj = getattr(client_module, name)
        if inspect.isclass(obj) and issubclass(obj, NamespacedClient) and obj is not NamespacedClient:
            classes.append(obj)
    return classes


@pytest.mark.parametrize("client_class", _all_client_classes(), ids=lambda c: c.__name__)
def test_instrumented_methods(client_class):
