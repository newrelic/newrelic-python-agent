...
@validate_transaction_metrics("test_register:test_register_type", background_task=True)
@validate_transaction_errors(errors=[])
@background_task()
def test_register_type():
    with psycopg2.connect(
        ...
        numeric_type = psycopg2.extensions.new_type(
            (1700,), "DEC2FLOAT", lambda v, _: float(v) if v is not None else None
        )
        psycopg2.extensions.register_type(numeric_type, cursor)

        numeric_array_type = psycopg2.extensions.new_array_type((1231,), "DEC2FLOATARRAY", numeric_type)
        psycopg2.extensions.register_type(numeric_array_type, cursor)
