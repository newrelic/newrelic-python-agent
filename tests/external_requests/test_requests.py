...
def metrics(server):
    scoped = [(f"External/localhost:{int(server.port)}/requests/", 1)]

    rollup = [
        ("External/all", 1),
        ("External/allOther", 1),
        (f"External/localhost:{int(server.port)}/all", 1),
        (f"External/localhost:{int(server.port)}/requests/", 1),
    ]
...
def test_http_request_get(server, metrics):
    ...
    def _test():
        requests.get(f"http://localhost:{int(server.port)}/")
...
def test_https_request_get(server, metrics):
    ...
    def _test():
        try:
            requests.get(f"https://localhost:{int(server.port)}/", verify=False)  # nosec
        except Exception:
            pass
...
def test_http_session_send(server, metrics):
    ...
    def _test():
        ...
        req = requests.Request("GET", f"http://localhost:{int(server.port)}/")
