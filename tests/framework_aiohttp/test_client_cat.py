...
def test_outbound_cross_process_headers(event_loop, cat_enabled, distributed_tracing, span_events, mock_header_server):
    ...
    async def _test():
        headers = await fetch(f"http://127.0.0.1:{int(mock_header_server.port)}")
...
def test_outbound_cross_process_headers_custom_headers(event_loop, customer_headers, mock_header_server):
    headers = event_loop.run_until_complete(
        background_task()(fetch)(f"http://127.0.0.1:{int(mock_header_server.port)}", customer_headers.copy())
    )
...
def test_outbound_cross_process_headers_no_txn(event_loop, mock_header_server):
    headers = event_loop.run_until_complete(fetch(f"http://127.0.0.1:{int(mock_header_server.port)}"))
...
def test_process_incoming_headers(
    ...
    address = f"http://127.0.0.1:{int(server.port)}"
    port = server.port

    _test_cross_process_response_scoped_metrics = [
        (f"ExternalTransaction/127.0.0.1:{int(port)}/1#2/test", 1 if cat_enabled else None)
    ]

    _test_cross_process_response_rollup_metrics = [
        ("External/all", 1),
        ("External/allOther", 1),
        (f"External/127.0.0.1:{int(port)}/all", 1),
        (f"ExternalApp/127.0.0.1:{int(port)}/1#2/all", 1 if cat_enabled else None),
        (f"ExternalTransaction/127.0.0.1:{int(port)}/1#2/test", 1 if cat_enabled else None),
    ]
