...
@validate_transaction_metrics(
    "views:index",
    scoped_metrics=[("Function/views:index", 1)] + scoped_metrics_wildcard_exclude_specific_include,
    rollup_metrics=scoped_metrics_wildcard_exclude_specific_include + [(f"Python/Framework/Django/{django.get_version()}", 1)]
)
@validate_code_level_metrics("views", "index")
def test_wildcard_exclude_specific_include_filter(application):
