...
def test_basic(app):
    ...
    @validate_transaction_metrics(
        "_target_application:Index.on_get", scoped_metrics=_test_basic_metrics, rollup_metrics=_test_basic_metrics
    )
    def _test():
...
def test_unhandled_exception(app):
    falcon_version = get_package_version_tuple("falcon")

    # Falcon v3 and above will not raise an uncaught exception
    if falcon_version[0] >= 3:
