...
def test_distributed_tracing_headers(topic, producer, consumer, serialize, expected_broker_metrics):
    ...
    def _consume():
        ...
        # The headers are eventually accepted (as seen by the
        # suppoortability metrics) but they are done out of order
        # due to the fact that OTel usually calls extract before
        # creating a span.  The issue here is that this may be
        # outside the context of a transaction, so the headers
        # will not be available when the record is created.
        def _test():
