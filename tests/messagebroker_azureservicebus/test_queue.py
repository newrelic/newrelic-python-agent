...
def test_queue_send_and_receive(queue_name, queue_sender, queue_receiver):
    _metrics = [
        ...
        (f"MessageBroker/ServiceBus/Queue/Peek/Named/{queue_name}", 1),
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_send_and_receive.<locals>._test",
    ...
    def _test():
        ...
        received_peek_messages = queue_receiver.peek_messages(max_message_count=1)
...
def test_queue_schedule_and_cancel(queue_name, queue_sender):
    ...
    def _test():
        ...
        sequence_number = queue_sender.schedule_messages(sent_message, scheduled_time_utc)
        queue_sender.cancel_scheduled_messages(sequence_number)

    _test()


def test_queue_send_and_receive_deferred_message(queue_name, queue_sender, queue_receiver):
    _metrics = [
        (f"MessageBroker/ServiceBus/Queue/Produce/Named/{queue_name}", 1),
        (f"MessageBroker/ServiceBus/Queue/Consume/Named/{queue_name}", 1),
        (f"MessageBroker/ServiceBus/Queue/Settle/Named/{queue_name}", 2),
    ]

    @validate_transaction_metrics(
        "test_queue:test_queue_send_and_receive_deferred_message.<locals>._test",
        scoped_metrics=_metrics,
        rollup_metrics=_metrics,
        background_task=True,
    )
    @background_task()
    def _test():
        message_text = "Send message from queue to be deferred."
        sent_message = ServiceBusMessage(message_text)
        queue_sender.send_messages(sent_message)
        ...
        for message in received_messages:
            ...
            queue_receiver.defer_message(message)

        received_deferred_messages = queue_receiver.receive_deferred_messages(sequence_numbers=sequence_number)
...
def test_queue_send_and_receive_dead_letter_message(queue_name, queue_sender, queue_receiver, queue_dead_letter_receiver):
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_send_and_receive_dead_letter_message.<locals>._test",
        scoped_metrics=_metrics,
        rollup_metrics=_metrics,
        background_task=True,
    )
    @background_task()
    def _test():
        message_text = "Send message from queue to dead letter queue."
        sent_message = ServiceBusMessage(message_text)
        queue_sender.send_messages(sent_message)
        ...
        for message in received_dead_letter_messages:
            message_body = message._message.data[0].decode()
            assert message_body == message_text
            queue_dead_letter_receiver.complete_message(message)

    _test()


def test_queue_send_and_receive_AmqpAnnotatedMessage(queue_name, queue_sender, queue_receiver):
    _metrics = [
        (f"MessageBroker/ServiceBus/Queue/Produce/Named/{queue_name}", 1),
        (f"MessageBroker/ServiceBus/Queue/Consume/Named/{queue_name}", 1),
        (f"MessageBroker/ServiceBus/Queue/Settle/Named/{queue_name}", 1),
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_send_and_receive_AmqpAnnotatedMessage.<locals>._test",
        scoped_metrics=_metrics,
        rollup_metrics=_metrics,
        background_task=True,
    )
...
def test_queue_send_and_receive_multiple_messages(queue_name, queue_sender, queue_receiver):
    _metrics = [
        (f"MessageBroker/ServiceBus/Queue/Produce/Named/{queue_name}", 1),
        (f"MessageBroker/ServiceBus/Queue/Consume/Named/{queue_name}", 2),
        (f"MessageBroker/ServiceBus/Queue/Settle/Named/{queue_name}", 2),
    ]

    @validate_transaction_metrics(
        "test_queue:test_queue_send_and_receive_multiple_messages.<locals>._test",
        scoped_metrics=_metrics,
        rollup_metrics=_metrics,
        background_task=True,
    )
    ...
    def _test():
        ...
        received_messages = queue_receiver.receive_messages(max_message_count=2, max_wait_time=5)
        for message in received_messages:
            message_body = message._message.data[0].decode()
...
def test_queue_distributed_traces_one_sent_one_received(queue_name, queue_sender, queue_receiver):
    """
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_distributed_traces_one_sent_one_received.<locals>._receive",
...
def test_queue_distributed_traces_one_sent_two_received(queue_name, queue_sender, queue_receiver):
    """
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_distributed_traces_one_sent_two_received.<locals>._receive1",
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_distributed_traces_one_sent_two_received.<locals>._receive2",
...
def test_queue_send_and_receive_iterative(queue_name, queue_sender, queue_receiver):
    ...
    @validate_transaction_metrics(
        "test_queue:test_queue_send_and_receive_iterative.<locals>._test",
        scoped_metrics=_metrics,
        rollup_metrics=_metrics,
        background_task=True,
    )
