...
new_pika_xfail = pytest.mark.xfail(
    condition=pika_version_info[0] > 0, strict=True, reason="test fails if pika version is 1.x or greater"
)
...
new_pika_skip = pytest.mark.skipif(
    condition=pika_version_info[0] > 0, reason="test hangs if pika version is 1.x or greater"
)
