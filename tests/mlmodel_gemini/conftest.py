...
def gemini_server(
    gemini_version,  # noqa: F811
    gemini_clients,
    wrap_gemini_api_requestor_request,
    wrap_gemini_api_requestor_interpret_response,
...
def wrap_httpx_client_send(extract_shortened_prompt):  # noqa: F811
    def _wrap_httpx_client_send(wrapped, instance, args, kwargs):
        ...
        if stream:
            GEMINI_AUDIT_LOG_CONTENTS[prompt] = [headers, response.status_code, []]  # Append response data to log
            if prompt == "error":
