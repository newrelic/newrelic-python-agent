...
@validate_span_events(
    exact_agents={
        "error.message": "Must provide an 'engine' or 'model' parameter to create a <class 'gemini.api_resources.chat_completion.ChatCompletion'>",
    }
)
@validate_transaction_metrics(
...
@validate_span_events(
    exact_agents={
        "error.message": "Incorrect API key provided: invalid. You can find your API key at https://cloud.google.com/vertex-ai/generative-ai.",
    }
)
@validate_transaction_metrics(
...
@validate_span_events(
    exact_agents={
        "error.message": "No API key provided. You can set your API key in code using 'gemini.api_key = <API-KEY>', or you can set the environment variable OPENAI_API_KEY=<API-KEY>). If your API key is stored in a file, you can point the openai module at it with 'openai.api_key_path = <PATH>'. You can generate API keys in the OpenAI web interface. See https://platform.openai.com/account/api-keys for details.",
    }
)
@validate_transaction_metrics(
