...
@background_task()
def test_text_generation_invalid_request_error_no_model(set_trace_info):
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_invalid_request_error_no_model_no_content(set_trace_info):
    with pytest.raises(TypeError):
        set_trace_info()
        add_custom_attribute("llm.conversation_id", "my-awesome-id")
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_invalid_request_error_invalid_model_with_token_count(set_trace_info):
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_wrong_api_key_error(monkeypatch, set_trace_info):
    with pytest.raises(google.genai.errors.ClientError):
        set_trace_info()
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_async_invalid_request_error_no_model(loop, set_trace_info):
    with pytest.raises(TypeError):
        set_trace_info()
        add_custom_attribute("llm.conversation_id", "my-awesome-id")
        with WithLlmCustomAttributes({"context": "attr"}):
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_async_invalid_request_error_no_model_no_content(loop, set_trace_info):
    with pytest.raises(TypeError):
        set_trace_info()
        add_custom_attribute("llm.conversation_id", "my-awesome-id")
        loop.run_until_complete(
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_async_invalid_request_error_invalid_model_with_token_count(loop, set_trace_info):
    with pytest.raises(google.genai.errors.ClientError):
        set_trace_info()
        add_custom_attribute("llm.conversation_id", "my-awesome-id")
        loop.run_until_complete(
...
@validate_custom_event_count(count=2)
@background_task()
def test_text_generation_async_wrong_api_key_error(loop, monkeypatch, set_trace_info):
    with pytest.raises(google.genai.errors.ClientError):
        set_trace_info()
        bad_client = google.genai.Client(api_key="DEADBEEF")
        loop.run_until_complete(
