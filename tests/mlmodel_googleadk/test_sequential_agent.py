...
EXPECTED_METRICS = [
    ...
    (f"Llm/agent/GoogleADK/run_async/{STEP2_AGENT_NAME}", 1),
]
...
EXPECTED_STEP2_SUBCOMPONENT = f'{{"type": "APM-AI_AGENT", "name": "{STEP2_AGENT_NAME}"}}'
...
STEP2_SPAN_NAME = f"Llm/agent/GoogleADK/run_async/{STEP2_AGENT_NAME}"
...
@validate_custom_events(events_with_context_attrs(sequential_recorded_events))
@validate_custom_event_count(EXPECTED_EVENT_COUNT)
@validate_transaction_metrics(
    "test_sequential_agent:test_sequential_agent",
...
@background_task()
def test_sequential_agent(exercise_agent, set_trace_info):
    set_trace_info()
    agent = build_sequential()
    with WithLlmCustomAttributes({"context": "attr"}):
        events = exercise_agent(agent, SEQUENTIAL_PROMPT)
...
@reset_core_stats_engine()
@disabled_ai_monitoring_record_content_settings
@validate_custom_events(sequential_recorded_events)
@validate_custom_event_count(EXPECTED_EVENT_COUNT)
@validate_transaction_metrics(
    "test_sequential_agent:test_sequential_agent_no_content",
    scoped_metrics=EXPECTED_METRICS,
    rollup_metrics=EXPECTED_METRICS,
    custom_metrics=EXPECTED_VERSION_METRICS,
    background_task=True,
...
@background_task()
def test_sequential_agent_no_content(exercise_agent, set_trace_info):
    set_trace_info()
    agent = build_sequential()
    events = exercise_agent(agent, SEQUENTIAL_PROMPT)
...
@validate_transaction_metrics(
    "test_sequential_agent:test_sequential_agent_disabled_ai_monitoring", background_task=True
)
@background_task()
def test_sequential_agent_disabled_ai_monitoring(exercise_agent, set_trace_info):
    set_trace_info()
    agent = build_sequential()
    events = exercise_agent(agent, SEQUENTIAL_PROMPT)
...
@reset_core_stats_engine()
@validate_custom_event_count(0)
def test_sequential_agent_outside_transaction(exercise_agent, set_trace_info):
    set_trace_info()
    agent = build_sequential()
    events = exercise_agent(agent, SEQUENTIAL_PROMPT)
