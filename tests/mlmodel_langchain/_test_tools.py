...
@pytest.fixture
def tool_method_metric(tool_type):
    invoked_method = "run" if tool_type == "sync_tool" else "arun"
    return (f"Llm/tool/LangChain/{invoked_method}/add_exclamation", 1)


@pytest.fixture
def add_exclamation(tool_type, exercise_method):
    if tool_type == "sync_tool":
        return add_exclamation_sync
    elif tool_type == "async_tool":
        if exercise_method in ("invoke", "stream", "stream_events"):
