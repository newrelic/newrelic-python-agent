...
def function_step(state):
    return f"The real agent said: {state['output']}"


@reset_core_stats_engine()
@validate_transaction_metrics(
    name="test_runnableseq:test_runnableseq_invoke",
...
@background_task()
def test_runnableseq_invoke(chat_openai_client, tools, prompt):
    agent = create_openai_functions_agent(chat_openai_client, tools, prompt)
    agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)
    seq = RunnableSeq(agent_executor, function_step)
    response = seq.invoke({"input": "Hello, world"})
    assert response == 'The real agent said: The extracted information from the input "Hello, world" is "Helloworld"'
