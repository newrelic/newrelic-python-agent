...
@validate_transaction_metrics(
    name="test_stategraph:test_stategraph_invoke",
...
@background_task()
def test_stategraph_invoke(chat_openai_client, tools, prompt):
    agent = create_openai_functions_agent(chat_openai_client, tools, prompt)
    agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True, name="agent_executor")
    ...
    response = graph.invoke({"input": "Hello, world"})

    assert (
        response["output"]
        == 'The real agent said: The extracted information from the input "Hello, world" is "Helloworld"'
    )
