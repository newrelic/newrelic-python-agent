...
def handler(environ, start_response):
    ...
    f = httplib2.HTTPConnectionWithTimeout('newrelic.com', 80)
    f.connect()
    f.close()

    f = httplib2.HTTPSConnectionWithTimeout('www.google.com', 443)
    f.connect()
    f.close()

    f = httplib2.Http()
    f.request('http://rpm.newrelic.com/')

    f = httplib2.Http()
    f.request('https://bing.com/')
